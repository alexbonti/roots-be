...
var getTokenFromDB = async function (userId, userType, token) {
  ...
  switch (userType) {
    case Config.APP_CONSTANTS.DATABASE.USER_ROLES.USER:
      result = await Services.UserService.getUserPromise(
        criteria,
        {},
        {}
      );
    ...
    case Config.APP_CONSTANTS.DATABASE.USER_ROLES.EMPLOYER:
      result = await Services.EmployerService.getEmployerPromise(
        criteria,
        {},
        {}
      );
      ...
      function (cb) {
        switch (userType) {
          case Config.APP_CONSTANTS.DATABASE.USER_ROLES.USER:
            ...
            break;
            case Config.APP_CONSTANTS.DATABASE.USER_ROLES.EMPLOYER:
            Services.EmployerService.updateEmployer(
              criteria,
              setQuery,
              { new: true },
              function (err, dataAry) {
